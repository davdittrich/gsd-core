...
- [ ] **(Capability entries only)** `interactions.loopExtensionPoints` is a non-empty subset of the 13 Loop Extension Points, `interactions.hookKinds` ⊆ `{step, contribution, gate}`, and `interactions.configKeys` / `requires` / `runtimeCompat` / `produces` / `consumes` are present (empty arrays are fine where nothing applies)
