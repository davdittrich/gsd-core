...
```bash
VERIFY_PRE_HOOKS_JSON=$(gsd_run loop render-hooks verify:pre --raw --phase "${phase_number}" ${GSD_WS})
...
```bash
VERIFY_POST_HOOKS_JSON=$(gsd_run loop render-hooks verify:post --raw --phase "${phase_number}" ${GSD_WS})
