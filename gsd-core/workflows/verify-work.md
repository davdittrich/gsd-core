...
```bash
VERIFY_PRE_HOOKS_JSON=$(gsd_run loop render-hooks verify:pre --phase "${phase_number}" ${GSD_WS} --raw)
...
```bash
VERIFY_POST_HOOKS_JSON=$(gsd_run loop render-hooks verify:post --phase "${phase_number}" ${GSD_WS} --raw)
