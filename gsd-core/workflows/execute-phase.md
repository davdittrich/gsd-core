   ...
   Agent(
    ...
    description="Execute plan {plan_number} of phase {phase_number}",
     ...
     prompt="
       ...
       Before choosing the conditional `tdd.md` entry, read the selected PLAN.md from the repository root at `{phase_dir}/{plan_file}`. Ignore an optional BOM before leading YAML frontmatter. Set `PLAN_TDD_CONTEXT=true` only when that frontmatter has `type: tdd` or a quoted scalar `tdd`, or an actual task opening tag (which may be multiline) at the start of a non-fenced line has `tdd="true"` (or another quoted true form; whitespace around `=` is allowed). Literal task examples in prose or fenced code blocks opened by backticks or tildes do not qualify. If that PLAN.md cannot be read, halt this dispatch. If `PLAN_TDD_CONTEXT=true` and `tdd.md` cannot be read, halt this dispatch. After completing this build-time composition and before calling `Agent()`, if literal `${PLAN_TDD_CONTEXT` survives, halt this dispatch rather than sending an unresolved marker to an executor.
