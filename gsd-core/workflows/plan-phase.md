...
if [ -z "${PLAN_PRE_HOOKS_JSON:-}" ]; then
  PLAN_PRE_HOOKS_JSON=$(gsd_run loop render-hooks plan:pre --raw --phase "$PHASE" ${GSD_WS})
fi
```
...
```bash
PLAN_PRE_HOOKS_JSON=${PLAN_PRE_HOOKS_JSON:-$(gsd_run loop render-hooks plan:pre --raw --phase "$PHASE" ${GSD_WS})}
...
```bash
PLAN_POST_HOOKS_JSON=$(gsd_run loop render-hooks plan:post --raw --phase "$PHASE" ${GSD_WS})
