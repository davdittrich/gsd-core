...
if [ -z "${PLAN_PRE_HOOKS_JSON:-}" ]; then
  PLAN_PRE_HOOKS_JSON=$(gsd_run loop render-hooks plan:pre --phase "$PHASE" ${GSD_WS} --raw)
fi
```
...
```bash
PLAN_PRE_HOOKS_JSON=${PLAN_PRE_HOOKS_JSON:-$(gsd_run loop render-hooks plan:pre --phase "$PHASE" ${GSD_WS} --raw)}
...
```bash
PLAN_POST_HOOKS_JSON=$(gsd_run loop render-hooks plan:post --phase "$PHASE" ${GSD_WS} --raw)
