...
`/gsd-code-review` opts into optional external reviewer lanes directly: its `dispatch_reviewer_lanes` step (`gsd-core/workflows/code-review.md`) matches the reviewer-lane roster (`review-lane-descriptor.cjs`) against the user's own CLI flags (e.g. `--codex`, `--agy`) and, only when at least one matches, calls `gsd_run review-lane dispatch-step` — the ONE shared interpreter, `dispatchReviewerLanes` (`src/reviewer-step-dispatch.cts`) — which owns selection, per-lane planning, and invocation, giving `/gsd-code-review` exact parity with the hand-driven `review-lane plan`/`invoke` calls `/gsd-review` already uses, instead of a second implementation. The interpreter never embeds file contents in a lane's prompt — only the repository root, canonical file paths, review depth, base SHA, and four fixed, non-negotiable prohibitions (no source mutation, no test execution, no background processes, no polling). `gsd-code-reviewer` is the sole consolidator: it independently re-verifies every external claim against the actual source under full-context review scope (surrounding modules, callers, tests, docs, config) before writing anything to `REVIEW.md`, so a lane's evidence is corroborating input, never a second output schema. No reviewer-lane flag on the command line means zero selection, plan, or invoke calls — this path is unrelated to the generic loop-hook-dispatch mechanism described above.
