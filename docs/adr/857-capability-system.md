...
- **Amended by:** [ADR-3997](3997-review-prompt-capability-contributions.md) (Review-Prompt Capability Contributions) — adds a 13th loop extension point, `review:pre`, orthogonal to the five-step pipeline this ADR defines. "Loop Extension Points (the 12)" below is now 13; see ADR-3997 for the new point's role and rationale.
...
- **The Loop Extension Points are wired.** All 13 named points (`discuss:pre/post`, `plan:pre/post`, `execute:pre`, `execute:wave:pre/post`, `execute:post`, `verify:pre/post`, `ship:pre/post`, `review:pre` — ADR-3997) have live render-hook call sites in the host loop.
