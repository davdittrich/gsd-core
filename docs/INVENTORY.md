...
| `loop-host-contract.cjs` | Generated Loop Host Contract — 13 loop points, per-step agent roles, and core artifacts for the five-step pipeline (discuss/plan/execute/verify/ship) plus the orthogonal review host; emitted by `scripts/gen-loop-host-contract.cjs --write` (ADR-894 §3); consumed by `gen-capability-registry.cjs` |
