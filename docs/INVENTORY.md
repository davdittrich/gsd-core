...
| `task-command-router.cjs` | Thin CJS subcommand router adapter for `gsd-tools task`; `resolve-content` subcommand resolves a task's `tracker-id` via `task-content-resolution.cjs` (ADR-3646, #3970); `red-evidence-verdict` subcommand dispatches `red-evidence-predicate.cjs`, uses `--red-sha` to derive the RED commit's sole parent and exact NUL-delimited changed paths from Git, and requires those paths to include the selected `<red_contract>`'s `target_test`. Security-flag cardinality failures return typed before claim; a valid-shape `--red-sha` is checked after claim/read; consumption is attempted on terminal verdict paths after a successful claim, with inability to consume returning typed fail-closed (D-1, #3770 — see that module's row below) |
...
| `red-evidence-predicate.cjs` | RED-evidence predicate for the MVP+TDD gate (compiled from `src/red-evidence-predicate.cts`, gitignored; #3770) — leaf-pure evaluator (no fs, no child_process, no config) implementing tdd.md's `### RED Predicate` fence: exact six-key trailer set equality, `expected`/`actual` three-key set equality, `location.declared`/`location.observed` shape and agreement (`path.win32.basename` separator split, strict line, never a column), `trailer.expected == plan.expected_failure`, `trailer.target_test == plan.target_test`, and a single conjunction ending in `actual.subject == plan.target_test`. Returns the three-valued verdict `authorize` / `red_commit_not_failing` / `unexpected_pass` with a bounded `reason` naming the deciding conjunct; fails closed on an absent, malformed, or over-shaped record. Dispatched behind `task-command-router.cjs`'s `red-evidence-verdict` subcommand, which uses `--red-sha` to derive the RED commit's sole parent and exact NUL-delimited changed paths from Git, refuses an otherwise-authorizing trailer when those paths omit the selected `<red_contract>`'s `target_test` (D-1), and attempts to consume each successfully claimed receipt on every terminal path, then fails closed if deletion cannot complete — a post-selection membership check, not a search filter — before the gate block in `gsd-core/workflows/execute-phase.md` authorizes GREEN |
